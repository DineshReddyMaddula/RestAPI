...
Module.prototype.findId = function(id,callback){
    this.db.query('SELECT * FROM subject'+this.id+ id +this.category+ id,function(err,data){
        if(err){
            callback(err,null);
        }else{
            callback(err,data);
...
Module.prototype.change = function(id,body,callback){
    this.db.query('UPDATE subject SET ? '+this.id+ id +this.category+ id,body,function(err,data){
        if(err){
            callback(err,null);
        }else{
            callback(err,body);
...
Module.prototype.remove = function(id,callback){
    this.db.query('DELETE FROM subject'+this.id+ id +this.category+ id,function(err,data){
        if(err){
            callback(err,null);
        }else{
            callback(err,data);
